...
theta_2 = cell2mat(wheelFile(2:end,9));
...
I_drum = inertiaCalc(m_coin(drum_ind), r_coin(drum_ind), theta_1(drum_ind),char(name_wheel(drum_ind)));
...
for i = 1:length(test_tires)
    ...
    I_wheel = inertiaCalc(m_coin(tire_ind), r_coin(tire_ind), theta_1(tire_ind),char(name_wheel(tire_ind)));
    %I_wheel = 0.05; 
